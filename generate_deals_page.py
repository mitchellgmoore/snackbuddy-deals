...
document.addEventListener("DOMContentLoaded", function () {{
  ...
  function initWelcomeFlow() {{
    ...
    // Email / digest CTAs use ?from=email so readers land on deals, not the
    // subscribe onboarding sheet.
    let fromEmail = false;
    try {{
      const params = new URLSearchParams(window.location.search || "");
      fromEmail = params.get("from") === "email" || params.get("skip_welcome") === "1";
    }} catch (err) {{}}
    if (fromEmail) {{
      ...
      applyWelcomePrefs(loadWelcomePrefs() || {{}});
      return;
    }}
